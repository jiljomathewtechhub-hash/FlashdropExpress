...
export const CustomerPortal: React.FC<CustomerPortalProps> = ({ onNavigate }) => {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProfileSuccessMsg('Profile updated successfully! Your permanent default address and tax details are saved and will auto-fill your delivery requests without ever being overwritten.');
      ...
      <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center space-x-5">
          ...
          <div>
            ...
            {/* Default Pickup Address Quick Indicator */}
            <div className="mt-2.5 flex flex-wrap items-center gap-2 text-xs">
              <span className="inline-flex items-center text-slate-700 bg-slate-100/90 border border-slate-200/90 px-3 py-1 rounded-lg">
                <MapPin className="w-3.5 h-3.5 text-red-600 mr-1.5 shrink-0" />
                <span className="font-medium">
                  Default Pickup:{' '}
                  <strong className="text-slate-900 font-semibold">
                    {user.defaultPickupAddress || 'Not configured'}
                  </strong>
                  {user.defaultPickupUnit ? ` (${user.defaultPickupUnit})` : ''}
                </span>
              </span>
              <button
                type="button"
                onClick={() => setActiveTab('profile')}
                className="text-[11px] font-bold text-red-700 hover:text-red-800 hover:underline inline-flex items-center space-x-0.5 cursor-pointer ml-1"
              >
                <span>{user.defaultPickupAddress ? 'Edit Address' : 'Set Default'}</span>
                      ...
                      <div className="md:col-span-8 bg-slate-50 rounded-xl p-3.5 border border-slate-100 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1.5 border-t border-slate-200/60 font-medium">
                          <span className="flex items-center space-x-1">
                            ...
                            <span>{ord.service_area === 'GTA' ? 'Greater Toronto Area (GTA)' : (ord.service_area || 'Regional Delivery Coverage')}</span>
                          </span>
            ...
            <form onSubmit={handleAddLocation} className="bg-white border-2 border-red-200 rounded-3xl p-6 shadow-md space-y-4 text-xs animate-fade-in">
              ...
              <div>
                ...
                <AddressAutocompleteInput
                  value={newLocAddress}
                  onChange={(val) => setNewLocAddress(val)}
                  onSelect={(sug) => setNewLocAddress(sug.fullAddress)}
      ...
      {activeTab === 'profile' && (
        <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6 max-w-3xl">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-5 text-xs">
            ...
            {/* Permanent Default Pickup Facility Card */}
            <div className="bg-slate-50/90 border border-slate-200 rounded-2xl p-4 sm:p-5 space-y-3 shadow-2xs">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 text-xs sm:text-sm">
                    Permanent Default Pickup Address &amp; Warehouse
                  </h4>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    This is your permanent company facility address. It will auto-fill your delivery requests to save you time. One-off delivery orders will <strong className="text-slate-800">never</strong> change or overwrite this saved address.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 pt-2">
                <div className="sm:col-span-2">
                  ...
                  <AddressAutocompleteInput
                    ...
                    onSelect={(sug) => {
                      isFormDirtyRef.current = true;
                      setEditAddress(sug.fullAddress);
                    }}
                    placeholder="Search address (e.g. 1450 Dundas St E, Mississauga, ON)..."
                    className="w-full text-xs"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 px-3 py-2 text-slate-900 rounded-xl focus:border-red-600 focus:bg-white focus:outline-none"
                  />
                </div>
              </div>
            </div>
